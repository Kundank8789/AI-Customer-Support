(function () {
    const api_Url = "https://ai-customer-support-navy.vercel.app/api/chat"

    const scriptTag = document.currentScript;
    const ownerId = scriptTag.getAttribute("data-owner-id")

    if (!ownerId) {
        console.log("Owner ID is required")
        return;
    }
    ...
    Object.assign(button.style, {
        ...
        cursor: "pointer",
        fontSize: "22px",
        boxShadow: "0 15px 49px rgba(0,0,0,0.35)",
        zIndex: "999999",
    })
    ...
    sendBtn.onclick = async () => {
        ...
        try {
            ...
            addMessage(data || "somthing went wrong", "ai")
        } catch (error) {
            console.error(error);
            messagesArea.removeChild(typing);
            addMessage(data ||"somthing went wrong", "ai")
